            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await emailService.SendEmailAsync(model.Email, "Confirm your account",
                        $"Qeydiyyatdan keçmək üçün linkə keçid edin: <a href='{callbackUrl}'>link</a>");
                    return View(nameof(PasswordConfirm));
                }
                else
                {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    {
                        ModelState.AddModelError(string.Empty, "Email-i təsdiq etməmisiniz");
                        return View(model);
                    }
                }
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await emailService.SendEmailAsync(model.Email, "Reset Password",
                    $"Parolu bərpa etmək üçün linkə keçid edin: <a href='{callbackUrl}'>link</a>");
                return View("ForgotPasswordConfirmation");
            }
            return View(model);
        }
